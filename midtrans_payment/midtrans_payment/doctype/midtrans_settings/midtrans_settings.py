...
For midtrans payment status is Authorized
...
class midtransSettings(Document):
	...
	def setup_addon(self, settings, **kwargs):
		...
		"""
		url = "https://api.midtrans.com/v1/subscriptions/{0}/addons".format(kwargs.get('subscription_id'))
	...
	def get_payment_url(self, **kwargs):
		...
		return get_url("./integrations/midtrans_checkout?token={0}".format(integration_request.name))
		...
		try:
			...
			if resp.get("status") == "captured":
				...
				if resp.get("status") == "refunded":
					...
					# midtrans refunds the amount after authorizing the card details
				...
				frappe.log_error(str(resp), 'midtrans Payment not authorized')
	...
	def verify_signature(self, body, signature, key):
		...
		if not result:
			frappe.throw(_('midtrans Signature Verification Failed'), exc=frappe.PermissionError)
...
def get_order(doctype, docname):
	# Order returned to be consumed by midtrans.js
	doc = frappe.get_doc(doctype, docname)
	try:
		# Do not use run_method here as it fails silently
		return doc.get_midtrans_order()
	except AttributeError:
		frappe.log_error(frappe.get_traceback(), _("Controller method get_midtrans_order missing"))
		frappe.throw(_("Could not create midtrans order. Please contact Administrator"))
...
def order_payment_failure(integration_request, params):
	"""Called by midtrans.js on failure

	Args:
		integration_request (TYPE): Description
		params (TYPE): error data to be updated
	"""
	frappe.log_error(params, 'midtrans Payment Failure')
	params = json.loads(params)
	integration = frappe.get_doc("Integration Request", integration_request)
	integration.update_status(params, integration.status)
...
@frappe.whitelist(allow_guest=True)
def midtrans_subscription_callback():
	try:
